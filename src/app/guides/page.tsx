  ...
  return <>
    <main className="mx-auto max-w-[1200px] px-5 py-10 sm:py-14 lg:px-8">
      <section>
        ...
        <p className="text-xl font-semibold tracking-[-0.025em] text-[#0D7A5A] sm:text-2xl">Explorer par catégorie</p>
        <div className="mx-auto mt-4 grid max-w-5xl gap-4 md:grid-cols-3">
          {categories.map((slug) => {
            ...
            return <Link key={slug} href={`/guides/${slug}`} className="group rounded-md border border-[#E1E4E3] bg-white p-7 transition-colors hover:border-[#CADCD5] sm:p-8"><div className="flex items-start justify-between gap-4"><h3 className="text-xl font-semibold tracking-[-0.025em] text-[#171717]">{category.name}</h3><span className="text-[#0D7A5A]">↗</span></div><p className="mt-3 text-[13px] leading-6 text-[#666]">{category.description}</p><p className="mt-5 text-center text-[13px] italic text-[#8A8A8A]">{count} guides disponibles</p></Link>
          })}
        </div>
      </section>

      <section className="mt-16 sm:mt-20">
        <div className="mb-7"><p className="text-xl font-semibold tracking-[-0.025em] text-[#0D7A5A] sm:text-2xl">À la une</p><p className="mt-2 text-[13px] leading-6 text-[#666]">Retrouvez ici nos articles les plus consultés pour résoudre rapidement les problèmes les plus courants en impression 3D.</p></div>
